...
import { IconArrowRight, IconCheck, IconPass } from "@/components/icons";
import { passCosts, turnTotal } from "@/lib/engine";
 ...
 * fix a fat-fingered swipe before the points are banked. Passes are numbered
 * so it is obvious which ones ran past the free ones and cost a point.
 */
export default function Recap({
  ...
  const free = game.settings.skipLimit;
  const costs = passCosts(played, free);
  const total = turnTotal(played, free);
  const next = game.teams[(game.turnTeam + 1) % game.teams.length];
  const lastInRound = game.turnTeam === game.teams.length - 1;
  const gameEnds = lastInRound && game.settings.rounds !== null && game.round >= game.settings.rounds;
  const got = played.filter((p) => p.outcome === "got").length;
  const passes = played.length - got;
  const paid = costs.filter(Boolean).length;

  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col px-4 pb-28 pt-14">
      <div className="rounded-3xl px-5 py-6 text-center shadow-clay" style={{ background: s.tint }}>
        ...
        <p
          className="pop-in mt-1 font-display font-extrabold leading-none"
          style={{ fontSize: "5rem", color: total < 0 ? "var(--buzz)" : s.deep }}
        >
          {total > 0 ? `+${total}` : total}
        </p>
        <p className="mt-1 text-sm text-ink/60">
          {got} got · {passes} passed{paid > 0 ? ` · ${paid} cost a point` : ""}
        </p>
      </div>
        ...
        <ul className="mt-4 space-y-2">
          {played.map((p, i) => (
            <li key={i} className="flex items-center gap-3 rounded-2xl bg-white px-3 py-2 shadow-clay">
              ...
              {costs[i] && (
                <span className="shrink-0 rounded-full bg-buzz-tint px-2 py-0.5 text-[11px] font-extrabold text-buzz">
                  −1
                </span>
              )}
              <div className="flex gap-1 rounded-xl bg-well p-1" role="radiogroup" aria-label={`Outcome for ${p.card.w}`}>
                {(
                  [
                    ...
                    { o: "pass", Icon: IconPass, on: "bg-buzz text-white", label: "Passed" },
                  ] as const
                ).map(({ o, Icon, on, label }) => (
                  <button
                    key={o}
                    ...
                    className={`flex h-9 w-10 items-center justify-center rounded-lg transition-all ${
                      p.outcome === o ? on + " shadow-clay" : "text-ink/40"
                    }`}
                  >
                    <Icon size={18} />
                  </button>
      ...
      <p className="mt-3 text-center text-xs text-ink/45">
        Tap an icon to correct a mis-swipe.
        {free !== null && free > 0 ? ` The first ${free} passes are free.` : ""}
      </p>
