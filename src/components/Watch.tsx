...
import { IconBack, IconHand, IconLogo } from "@/components/icons";
...
/**
 * The referee's phone. A mirror of the card on the host's screen, a very
 * large red button, and the scores. Read-only apart from the button.
 */
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setMs(
        snap.endsAt === null
          ? snap.remainingMs
          : Math.max(0, snap.endsAt - Date.now()),
      );
    }, 200);
          ...
          {!snap ? (
            <div className="flex flex-1 flex-col items-center justify-center text-center">
              ...
              {waited && (
                <p className="mt-2 max-w-xs text-sm text-ink/55">
                  Nothing on this code yet. Check the four letters, and make
                  sure the host phone has the game open.
                </p>
              )}
            </div>
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  <p
                    className="mt-1 break-words font-display font-extrabold leading-[1.05]"
                    style={{
                ...
                <div className="flex flex-1 flex-col justify-center px-5 py-3">
                  ...
                  <ul className="mt-2 space-y-1.5">
                    {snap.card.t.map((t) => (
                      <li
                        key={t}
                        className="flex items-center justify-center gap-2 rounded-xl bg-buzz-tint/70 py-1.5 font-display text-lg font-bold"
                      >
                        <span
                          className="h-2 w-2 rounded-full bg-buzz"
                          aria-hidden
                        />
                        {t}
                      </li>
              ...
              <button
                ...
                style={{
                  boxShadow: "var(--lift), inset 0 -5px 0 rgba(0,0,0,0.18)",
                }}
              >
                <IconHand size={40} /> BUZZ
              </button>
            </>
          ) : (
            <div className="flex flex-1 flex-col justify-center">
              ...
              <p className="mt-1 text-center text-sm text-ink/55">
                {snap.rounds
                  ? `Round ${Math.min(snap.round, snap.rounds)} of ${snap.rounds}`
                  : `Round ${snap.round}`}
              </p>
              <div className="mt-6">
                <Scoreboard
                  teams={snap.teams}
                  current={snap.phase === "over" ? undefined : snap.turnTeam}
                  large
                />
              </div>
            </div>
